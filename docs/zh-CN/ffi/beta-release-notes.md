...
本文档用于说明当前 `v0.2.x / beta` 阶段，`luaskills` FFI 对外发布时最应该让接入方知道的事情。
...
当前 `v0.2.x / beta` 阶段，建议把发布面理解成下面四层：
...
- 多语言 SDK
  - 推荐接入层
  - TypeScript / Python / Go 宿主优先使用对应 SDK，避免重复手写 buffer 与 JSON FFI 包络
- 标准 C ABI
  - 低层正式宿主契约
...
- Rust 宿主优先 Rust API
- TypeScript / Python / Go 宿主优先对应 SDK
- 低层正式跨语言宿主优先标准 C ABI
...
- 当前版本更适合作为**受控宿主集成接口**
- Rust 宿主的主集成方式仍然是 Rust 直连
- 非 Rust 宿主优先使用 SDK；底层 FFI 继续作为 SDK、受控宿主和高级集成的 ABI 基础
...
- ROOT / PROJECT / USER 三层模型下，`ROOT` 是天然系统保护层，`protected_skill_ids` 已取消
...
- 标准 C ABI、公共 `_json` FFI 与多语言 SDK 已完成基础分层
...
`v0.2.x / beta` 阶段的 `luaskills` 已经具备“可联调、可示例化、可 SDK 化、可受控发布”的 FFI 基础，但它的定位仍然是 beta 宿主接入面，而不是完全收敛后的终版 ABI。
