...
-- 说明 / Notes:
-- 在 Windows + OpenSSL 3 预编译依赖下补充 crypt32/user32/advapi32 链接库，
-- 避免 luasec 链接 libcrypto/libssl 时出现未解析的系统符号。
-- Add crypt32/user32/advapi32 on Windows when linking against the prebuilt OpenSSL 3 libraries,
-- preventing unresolved system symbols from libcrypto/libssl during the luasec build.
      ...
      windows = {
         ...
         modules = {
            ssl = {
               ...
               libraries = {
                  "libssl", "libcrypto", "crypt32", "user32", "advapi32", "ws2_32"
               },
               incdirs = {
                  "$(OPENSSL_INCDIR)", "src/", "src/luasocket"
               },
