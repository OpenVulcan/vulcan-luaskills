...
if ($IsWindows -or $env:OS -eq "Windows_NT") {
    ...
    $env:DYLD_LIBRARY_PATH = "$ReleaseDir" + $(if ($env:DYLD_LIBRARY_PATH) { ":$env:DYLD_LIBRARY_PATH" } else { "" })
} else {
    $env:LD_LIBRARY_PATH = "$ReleaseDir" + $(if ($env:LD_LIBRARY_PATH) { ":$env:LD_LIBRARY_PATH" } else { "" })
}
python (Join-Path $ProjectRoot "examples\ffi\python\demo.py")
